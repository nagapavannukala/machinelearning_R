...
ib<- read.csv("IPLBAT.csv", header=TRUE)
View(ib)

?dist  ## to get help on distance function
d.euc <- dist(x=ib[,2:12], method = "euclidean") 
d.euc
...
plot(clus1, labels = as.character(ib[,2]))

## scale function standardizes the values
scaled.ib <- scale(ib[,2:12])
head(scaled.ib, 10)
d.euc
...
aggr = aggregate(ib[,-c(1,13, 14,15)],list(ib$Clusters),mean)
