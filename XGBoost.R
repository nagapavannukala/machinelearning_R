...
?xgboost
set.seed(100)
bst <- xgboost(data = dtrain, eta = 0.8,#eta is same as shrinkage or learning rate. Correct terminology is 'eta'
               max_depth = 2, 
               nround=25, 
               subsample = 0.7,# equivalent to train fraction
...
rankxg<-ROTable(df = df_train,target = "Target",probability = "prob")
View(rankxg)
...
View(rank_test)
KS_AUC(df = df_test,target = "Target",probability = "prob")#This KS is more accurate because it get's KS from continuous curves.Unlike before one where deciling is done
